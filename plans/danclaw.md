...
- [ ] `StandardMessage` data model defined (source, channel_ref, user_id, content, session_id)
...
- [ ] Attribution formatting is configurable per session
- [ ] Detaching from terminal removes the binding without affecting the Slack session
...
- [ ] Obsidian tool script(s) in `tools/`: read file, write file, search/list files in vault
...
- [ ] Every dispatcher action emits a telemetry event: permission checks, routing decisions, agent spawns, tool calls, session state changes, errors, fallbacks
- [ ] Events written to JSONL file (append-only) and stored in `telemetry_events` DB table
