...
- `DispatchResult` — frozen dataclass with `session_id`, `response` (text), `backend` (name of the backend that produced it), `agent_name` (name of the agent that handled the message), and `fanout_channels` (tuple of channel_refs bound to the session excluding the source channel, for delivering the response to other listeners following the session).
...
- `SocketServer(dispatcher, socket_path)` — asyncio Unix domain socket server that fronts the Dispatcher. Accepts newline-delimited JSON and writes back JSON responses. Tracks connected clients by their `channel_ref` for fanout push delivery. Supports these request types:
