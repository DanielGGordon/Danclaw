...
- `build_executor(backend_preference, *, timeout=None)` — factory function that maps a list of backend names (e.g. `["claude", "codex"]`) to executor instances and wraps them in a `FallbackExecutor`. Known backends: `"claude"` (ClaudeExecutor), `"codex"` (CodexExecutor), `"mock"` (MockExecutor). Raises `ValueError` for empty lists or unknown backend names. The optional `timeout` (seconds) is passed through to `ClaudeExecutor` and `CodexExecutor`. This is the primary way to construct an executor from an agent's `backend_preference` config field.
